...
For the shortest local proof path, see [docs/WALKTHROUGH.md](../docs/WALKTHROUGH.md).
...
1. primary real path: protected downstream MCP server mode via `MCP_TARGET_COMMAND` plus `MCP_TARGET_ARGS_JSON`
2. demo path: protected local read/search-shaped workflow via `examples/demo-target.js`
3. secondary standalone path: bundled MCP mode via `npx -y mcp-transport-firewall`

Repo-local demo path (after install):
