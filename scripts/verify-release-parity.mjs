...
const canonicalRepository = 'shleder/mcp-transport-firewall';
...
export const verifyReleaseParity = ({ pkg, env = process.env, readGitFn = readGit } = {}) => {
  ...
  if (env.GITHUB_REPOSITORY && env.GITHUB_REPOSITORY !== canonicalRepository) {
    mismatches.push(`GITHUB_REPOSITORY must be ${canonicalRepository}, got ${env.GITHUB_REPOSITORY}`);
  }

  const originUrl = readGitFn('config', '--get', 'remote.origin.url');
  if (!originUrl.includes(canonicalRepository)) {
    mismatches.push(`remote.origin.url must point to ${canonicalRepository}, got ${originUrl}`);
  }
