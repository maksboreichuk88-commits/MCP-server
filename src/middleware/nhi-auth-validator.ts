...
export const nhiAuthValidator = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!serverToken) {
    auditLogWithSIEM('AUTH_FAILURE', { reason: 'Fail-Closed: PROXY_AUTH_TOKEN not configured', ip: req.ip });
  ...
  if (!ServerTokenSchema.safeParse(serverToken).success) {
    auditLogWithSIEM('AUTH_FAILURE', { reason: 'Fail-Closed: Server token is invalid', ip: req.ip });
  ...
  if (!authHeader || typeof authHeader !== 'string' || !authHeader.startsWith('Bearer ')) {
    auditLogWithSIEM('AUTH_FAILURE', { reason: 'Missing or invalid Authorization header scheme', ip: req.ip });
  ...
  try {
    ...
    if (nhiPayload.token.length !== serverToken.length || nhiPayload.token !== serverToken) {
      auditLogWithSIEM('AUTH_FAILURE', { reason: 'Token mismatch', ip: req.ip });
      res.status(401).json({ error: { code: 'AUTH_FAILURE', message: 'Invalid authentication token.' } });
      return;
    }

    req.nhiScopes = nhiPayload.scopes;
    writeAuditLog('AUTH_SUCCESS', { ip: req.ip, scopes: nhiPayload.scopes });
    next();
  } catch (error: unknown) {
    auditLogWithSIEM('AUTH_FAILURE', { reason: 'Invalid NHI Base64 JSON token structure', ip: req.ip });
